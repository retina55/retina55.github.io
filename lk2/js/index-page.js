$(function() {
	...
	// Калькулятор инвестора js-calc3
	// переключение кнопок тип займа
	$('.js-calc3 .opendeposit__left-list-buttons .form_radio_btn input').on('click', function(e) {
		var currentProcent = $(this).attr('data-procent');
		$('.opendeposit__left-list-text p span').text(currentProcent+' %');		
	});

	// изменение range sum
	$('.js-calc3 input.sum').on('input', function(e) {
		$('.opendeposit__left-list-range1-text h2').text($(this).val());
	});

	// изменение range month
	$('.js-calc3 input.month').on('input', function(e) {
		$('.opendeposit__left-list-range2-text h2').text($(this).val());
	});
